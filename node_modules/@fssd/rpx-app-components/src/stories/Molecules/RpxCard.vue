<template>
  <div class="lsx-card-container">
    <div v-if="isPastDueStatus !== ''" class="lsx-card__order-due-status"
      ...
      <span class="due-status">{{ isPastDueStatus === 'pastdue' ? 'PAST DUE' : 'DUE SOON' }}</span>
    </div>
    <v-card class="lsx-card" :class="[{ 'lsx-card-expanded': isCardClicked }]">
      <v-card-actions class="lsx-card__actions">
        <div class="wrapper--content-top">
          <v-icon class="lsx-card__actions--icons">{{ computedIcon }}</v-icon>
          ...
          <div v-if="orderDetails.orderType?.toLocaleLowerCase() === 'web' && orderDetails.isPaid" class="d-flex">
            <span class="lsx-card__actions--payment-status">Paid</span>
          </div>
        </div>
      ...
      <div class="wrapper--content-middle">
        ...
        <p class="lsx-card-due-order">Due <span class="lsx-card-due-time-date">{{ orderDetails.dueDateTime
        }}</span>
        </p>
      </div>
...
.lsx-card__actions--line {
  width: 0.0625rem;
  height: 1.125rem;
  background: $colorCardHorizontalLine 0% 0% no-repeat padding-box;
  opacity: 1;
  margin-left: 0.25rem;
...
.lsx-card__actions__divider-line {
  ...
  width: 11.875rem;
  height: 0.125rem;
  background-color: $colorCardDividerLine;
  opacity: 1;
}
